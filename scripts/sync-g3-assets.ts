...
// Desk strips use *-vo public URLs (g3-demo.ts links them) and are sourced from
// the current -v3 VO burns. The unversioned files in the pack are the Jul 8
// originals, which predate the desk-crew rebrand and were never remastered.
for (const desk of ["capital", "creator", "work"]) {
  ...
  if (copyFile(src, path.join(demoRoot, "g3", `g3-desk-strip-${desk}-vo.mp4`))) copied++;
}
